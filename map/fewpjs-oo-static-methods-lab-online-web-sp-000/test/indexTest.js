describe( "Formatter", () => {
  ...
  describe( "stringifyArray", () => {
    ...
    it( "makes each number in the array a string", () => {
      expect( Formatter.stringifyArray( [1, 2, 3, 4] ) ).to.equal( ["1", "2", "3", "4"] )
      expect( Formatter.stringifyArray( [10, 20, 30, 40 ] ) ).to.equal( ["10", "20", "30", "40"] )
      expect( Formatter.stringifyArray( [365, 520, 450 ] ) ).to.equal( ["365", "520", "450"] )
    } )
  } )
  ...
  describe("capitalizeNames", () => {
    ...
    it("properly capitalizes each name", () => {
      expect( Formatter.capitalizeNames( ["john", "JACOB", "jinGleHeimer", "schmidt"] ) ).to.equal( ["John", "Jacob", "Jingleheimer", "Schmidt"] )
      expect( Formatter.capitalizeNames( ["dan", "WaLKeR", "california", "TEXAS", "Garth"] ) ).to.equal( ["Dan", "Walker", "California", "Texas", "Garth"] )
    })
  })
  ...
  describe("namesOnly", () => {
    ...
    it("returns an array of the names of people", () => {
      expect( Formatter.namesOnly( people ) ).to.equal( expected )
    })
  })
  ...
  const expected = ["Angelina Jolie can go to The Matrix", 
    ...
    describe("given one needs to be 17 to enter the matrix", () => {
      it("returns an array of eligibility strings for each person entering the matrix", () => {
        expect( Formatter.eligibleForMatrix( people ) ).to.equal( expected )
      })
    })
  ...
  const expected = ["<h1>Angelina Jolie</h1><h2>80</h2>", 
    ...
    it("returns an array of properly formatted html (names in h1 tags and titles in h2 tags)", () => {
      expect( Formatter.createHtml( people ) ).to.equal( expected )
    })
  })
